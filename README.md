...
- `POLL_ADMIN_SECRET` — секрет для админских маршрутов. Используется на странице `/admin/poll-points` и в API
  `/api/polls/voter-stats*`. Можно также задать устаревшую переменную `POLL_POINTS_ADMIN_SECRET` — значение будет
  использовано как запасной вариант.

## Админская страница и API статистики голосов

- **Маршрут** `/admin/poll-points` — защищённая страница с фильтрами, поиском по токенам, устройствам и списку опросов.
  Для доступа укажите `POLL_ADMIN_SECRET` через Basic Auth или query-параметр `admin_secret`. Секрет можно сохранить в
  браузере прямо на странице.
- **API**:
  ...
  - В обоих случаях обязательна передача админского секрета (Basic Auth, `X-Admin-Secret`, `X-Poll-Admin-Secret` или
    `admin_secret` в query/JSON).
