...
- **API**:
  - `GET /api/polls/voter-stats` — возвращает пагинированный список участников: `voter_token`, `device_label`,
    `total_points`, `earned_points_total` (только начисленные баллы), `filtered_points` (начисленные баллы по фильтру),
    `votes_count`, `last_vote_at` и массив голосов (poll_id, название, дата, фильм, баллы, время голосования).
