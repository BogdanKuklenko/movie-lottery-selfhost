...
- **`/routes`**: Папка для всей логики маршрутизации.
    - `main_routes.py`: Отвечает за маршруты, которые **отрисовывают HTML-страницы** (например, главная, история, библиотека).
      Содержит защищённый `/admin/poll-points`, отображающий статистику баллов по токенам.
    - `api_routes.py`: Отвечает за все **API-маршруты** (те, что возвращают данные в формате JSON для JavaScript).
      Здесь расположены `GET /api/polls/voter-stats` и `GET /api/polls/voter-stats/<token>` с проверкой админского секрета.
...
- **`/components`**: Стили для каждого конкретного компонента (`_buttons.css`, `_modal.css`, `_slider.css` и т.д.).
  Дополнены `_admin.css` со стилями таблиц и фильтров для `/admin/poll-points`.
...
- **`/pages`**: **"Сборщики" страниц.** Главные скрипты для каждой страницы (`history.js`, `library.js`). Они импортируют нужные API-функции и компоненты и связывают их воедино. `admin_poll_points.js` отвечает за фильтры, сортировку и отображение таблицы на новой админской странице.
