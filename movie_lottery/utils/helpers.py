...
def change_voter_points_balance(
    voter_token,
    delta,
    device_label=None,
    commit=False,
    include_earned=False,
):
    ...
    result_balance = profile.total_points or 0
    result_earned = profile.earned_points_total or 0

    if getattr(profile, '_is_fallback', False):
        return (result_balance, result_earned) if include_earned else result_balance
